...
def ejecutar_encontrar_pelicula_mas_larga(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
   ...
   # TODO: Completar
    if p1["duracion"] >= p2["duracion"] and p1["duracion"] >= p3["duracion"] and p1["duracion"] >= p4["duracion"] and p1["duracion"] >= p5["duracion"]:
        print("La película más larga es:", p1["nombre"], "con una duración de", p1["duracion"], "minutos.")
    elif p2["duracion"] >= p1["duracion"] and p2["duracion"] >= p3["duracion"] and p2["duracion"] >= p4["duracion"] and p2["duracion"] >= p5["duracion"]:
        print("La película más larga es:", p2["nombre"], "con una duración de", p2["duracion"], "minutos.")
    elif p3["duracion"] >= p1["duracion"] and p3["duracion"] >= p2["duracion"] and p3["duracion"] >= p4["duracion"] and p3["duracion"] >= p5["duracion"]:
        print("La película más larga es:", p3["nombre"], "con una duración de", p3["duracion"], "minutos.")
    elif p4["duracion"] >= p1["duracion"] and p4["duracion"] >= p2["duracion"] and p4["duracion"] >= p3["duracion"] and p4["duracion"] >= p5["duracion"]:
        print("La película más larga es:", p4["nombre"], "con una duración de", p4["duracion"], "minutos.")
    else:
        print("La película más larga es:", p5["nombre"], "con una duración de", p5["duracion"], "minutos.")
...
def ejecutar_consultar_duracion_promedio_peliculas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
   ...
   # TODO: Completar
    ...
    promedio_duracion = duracion_total // 5

   
    horas = promedio_duracion // 60
    minutos = promedio_duracion % 60
    duracion_formato = f"{horas:02d}:{minutos:02d}"

    
    print("La duración promedio de las películas es:", duracion_formato)
...
def ejecutar_encontrar_estrenos(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
   ...
   # TODO: Completar
    try:
        anio_referencia = int(input("Introduce el año para determinar estrenos: "))
    except ValueError:
        print("Año no válido.")
        return
    ...
    if p1["anio"] > anio_referencia:
        estrenos.append(p1["nombre"])
    if p2["anio"] > anio_referencia:
        estrenos.append(p2["nombre"])
    if p3["anio"] > anio_referencia:
        estrenos.append(p3["nombre"])
    if p4["anio"] > anio_referencia:
        estrenos.append(p4["nombre"])
    if p5["anio"] > anio_referencia:
        estrenos.append(p5["nombre"])

    
    if estrenos:
        print("Películas de estreno:", ", ".join(estrenos))
    else:
        print("No hay películas de estreno para el año proporcionado.")
...
def ejecutar_cuantas_peliculas_18_mas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
   ...
   # TODO: Completar
    ...
    # Mostrar resultado
    print(f"Hay {contador} películas con clasificación 18+ en la agenda.")
...
def ejecutar_reagendar_pelicula(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    nueva_hora = int(input("Ingrese la nueva hora para la pelicula (en formato 24h, solo la hora): "))
    nuevo_dia = input("Ingrese el nuevo dia para la pelicula: ")
    
    control_horario = input("Desea controlar el horario de las películas? (s/n): ")
    if control_horario.lower() == 's':
        control_horario = True
    else:
        control_horario = False

    # Verificación de si es posible reagendar
    # Asumo que hay una función 'reagendar_pelicula' en el módulo 'mod'
    resultado = mod.reagendar_pelicula(pelicula, nueva_hora, nuevo_dia, control_horario, p1, p2, p3, p4, p5)
    
    if resultado:
        print(f"La pelicula {nombre} ha sido reagendada para {nuevo_dia} a las {nueva_hora}:00 hrs.")
    else:
        print(f"No es posible reagendar la pelicula {nombre} a la hora y día seleccionados.")
...
def ejecutar_decidir_invitar(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    autorizacion = input("¿El invitado cuenta con la autorizacion de sus padres para ver la pelicula? (s/n): ")

    autorizacion_padres = True if autorizacion.lower() == 's' else False
    ...
    if puede_invitar:
        print(f"Puedes invitar a la persona a ver {nom_peli}.")
    else:
        print(f"No puedes invitar a la persona a ver {nom_peli}.")
